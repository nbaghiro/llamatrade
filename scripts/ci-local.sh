...
if [[ "$RUN_LINT" == "true" && "$RUN_BACKEND" == "true" ]]; then
    ...
    # Check for suppression comments (# noqa, # type: ignore, # pyright:)
    run_step "No suppression comments" ./scripts/check-no-suppressions.sh

    if [[ "$FIX_MODE" == "true" ]]; then
        run_step "Ruff lint (with fixes)" ruff check --fix --config pyproject.toml services/ libs/ tests/ scripts/ || true
        run_step "Ruff format" ruff format --config pyproject.toml services/ libs/ tests/ scripts/ || true
    else
        # --ignore-noqa ensures noqa comments don't suppress errors (fix the root cause instead)
        run_step "Ruff lint" ruff check --config pyproject.toml --ignore-noqa services/ libs/ tests/ scripts/
        run_step "Ruff format check" ruff format --config pyproject.toml --check services/ libs/ tests/ scripts/
    fi
