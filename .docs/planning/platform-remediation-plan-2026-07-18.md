...
- **GAP 16 stream contract test (12A)** — a proto `LedgerFill` round-trips through the event transport (in-memory `FakeTransport`) — publish → durable consumer-group consume → decode → translate — into the portfolio ingestion, exercising the transport + codec + translation together (not mocks). (`tests/test_fill_stream_contract.py`.)
...
| **backtest** (`:8830`) | Celery engine + beat reaper; shared `StrategySession` adapter (live=backtest parity); streamed bar fetch; Kafka progress topic; cooperative cancel; commission reconciliation; guarded terminal writes. | **Cleanest service.** All 2026-06-19 hardening present + tested, 80% gate, 327 tests. GAP 1 fixed. |
...
| **portfolio** (`:8860`) | Pure double-entry ledger kernel (balanced postings, FIFO lots, reservations); idempotent consumer-group ingestion (DLQ, quarantine, invariant freeze); drift policy; snapshot performance; reconciliation loop; all reads projection-backed. | **Strongest code in the repo.** GAP 1 fixed. Open: GAP 11/12/15/16 + new replica/leak/outage issues (§3). |
