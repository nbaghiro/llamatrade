...
```
Option A — API Gateway            ·  extra hop + single point of failure
┌───────────────┐        ╭───────────────────╮        ╭───────────────────╮
│    Browser    │  ──►   │ API Gateway :8000 │  ──►   │      Services     │
├───────────────┤        ├───────────────────┤        ├───────────────────┤
│ HTTP / JSON   │        │ validates JWT     │        │ auth · strategy   │
└───────────────┘        │ rate-limit · log  │        │ trading · …       │
                         ╰───────────────────╯        ╰───────────────────╯
```
...
```
Option B — Direct Communication   ·  ★ CHOSEN — no proxy hop · services scale alone
┌───────────────┐        ╭───────────────────╮
│    Browser    │        │ Services (direct) │
├───────────────┤        ├───────────────────┤
│ one JWT       │  ──►   │ auth      · 8810  │
│ per service   │  ──►   │ strategy  · 8820  │
└───────────────┘  ──►   │ trading   · 8850  │
                   ──►   │ …         · 88xx  │
                         ╰───────────────────╯
```
