...
from llamatrade_runtime.evaluation.conditions import evaluate_condition_safe
...
from llamatrade_runtime.indicators.library import PriceData, compute_all_indicators
...
class CompiledStrategy:
    ...
    def compile(cls, strategy: Strategy) -> CompiledStrategy:
        ...
        # Largest lookback across indicators + weight/filter/metric blocks; floored at 2 for crossovers.
        min_bars = max(max_lookback(strategy, get_max_lookback(indicators)), 2)
    ...
    def _compute_momentum_weights(
        ...
        """Rank by trailing return, keep the top ``:top``, and weight proportionally to that return.

        Negative returns are clipped to zero (a down asset carries no long momentum, and a raw
        proportional split blows up when returns straddle zero). If every survivor is non-positive,
        fall back to equal weight among them.
        """
        lookback = lookback or 90
        scores = {s: trailing_return(state.bar_history.get(s, []), lookback) for s in symbols}

        sorted_symbols = sorted(symbols, key=lambda s: scores.get(s, 0.0), reverse=True)
        selected = sorted_symbols[:top] if top and top < len(sorted_symbols) else sorted_symbols
        if not selected:
            return {}

        positive = {s: max(scores.get(s, 0.0), 0.0) for s in selected}
        total = sum(positive.values())
        if total <= 0:
            equal_weight = 100.0 / len(selected)
            return {s: equal_weight for s in selected}
        return {s: (positive[s] / total) * 100.0 for s in selected}
    ...
    def _compute_inverse_volatility_weights(
        ...
        A symbol with too little history or zero variance has no usable volatility estimate, so it
        is excluded (0 weight) rather than assigned a tiny σ that would dominate the block; the
        qualifying symbols renormalize to 100. If none qualify, fall back to equal weight.
        """
        ...
        for symbol in symbols:
            ...
            if len(bars) >= lookback:
                vol = return_volatility(bars, lookback)
                if vol > 0:
        ...
        result = {s: (v / total) * 100.0 for s, v in inv_vols.items()}
        for symbol in symbols:
    ...
    def _calculate_filter_scores(
        ...
        for symbol in symbols:
            ...
            if criterion == "momentum":
                scores[symbol] = trailing_return(bars, lookback)
            elif criterion == "volatility":
                scores[symbol] = return_volatility(bars, lookback) if len(bars) >= lookback else 0.0
            elif criterion == "volume":
                scores[symbol] = average_dollar_volume(bars, lookback)
            else:
                scores[symbol] = 0.0
