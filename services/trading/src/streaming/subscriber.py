"""Trading event subscriber for real-time updates off the Kafka-backed event bus.
...
cursor. The yielded items are the proto ``trading_pb2.OrderUpdate`` /
``trading_pb2.PositionUpdate`` messages the bus carries directly.
"""
...
from llamatrade_events import CURSOR_NEW, OrderEvents, PositionEvents, get_default_transport
...
class TradingEventSubscriber:
    """Tail-reads trading events (orders, positions) from the event bus.
...
def get_trading_event_subscriber() -> TradingEventSubscriber:
