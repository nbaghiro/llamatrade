...
import { backtestClient, strategyClient } from '../net';
...
export function formatMetrics(metrics: BacktestMetrics | undefined): Record<string, string> {
  ...
  return {
    ...
    averageWin: money(toNumber(metrics.averageWin)),
    averageLoss: money(toNumber(metrics.averageLoss)),
    startingCapital: money(toNumber(metrics.startingCapital)),
    endingCapital: money(toNumber(metrics.endingCapital)),
    peakCapital: money(toNumber(metrics.peakCapital)),
    totalCommission: money(toNumber(metrics.totalCommission)),
